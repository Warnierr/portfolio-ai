...
function NeonPulse() {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black overflow-hidden">
            <AnimatePresence>
                {flash && <motion.div initial={{ opacity: 0 }} animate={{ opacity: [0, 0.3, 0] }} exit={{ opacity: 0 }} transition={{ duration: 0.6, ease: "easeInOut" }} className="fixed inset-0 bg-[#39ff14] mix-blend-screen pointer-events-none z-50" />}
            </AnimatePresence>
            <div className="absolute left-0 top-0 bottom-0 w-24 flex flex-col justify-around items-center gap-8 py-10">
                {[0, 1, 2, 3].map(i => <motion.div key={`left-${i}`} animate={{ opacity: [0.4, 0.8, 0.4] }} transition={{ duration: 3 + i, repeat: Infinity, delay: i * 0.5, ease: "easeInOut" }} className="w-3 h-32 bg-[#39ff14] rounded-full opacity-60" style={{ filter: 'blur(1px)', boxShadow: '0 0 15px rgba(57,255,20,0.6)' }} />)}
            </div>
            <div className="absolute right-0 top-0 bottom-0 w-24 flex flex-col justify-around items-center gap-8 py-10">
